    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title" id="formModalLabel" x-text="$store.customer.state"></h5>
            </div>
            <div class="modal-body p-2">
                <form action="" id="customerCateForm" x-on:submit.prevent="$store.customer.crudAction($event)">
                    <div class="col">
                        {!! Form::label('name', 'Customer Name', []) !!}
                        {!! Form::text('name', null, ['class' => 'form-control mb-2', 'id' => 'name', 'x-model' => '$store.customer.customerData.name']) !!}
                    </div>
                    <div class="col">
                        {!! Form::label('phone', 'Phone', []) !!}
                        {!! Form::text('phone', null, ['class' => 'form-control mb-2', 'id' => 'phone', 'x-model' => '$store.customer.customerData.phone']) !!}
                    </div>
                </form>
            </div>
            <div class="modal-footer">
                <button type="button" id="closeModalBtn" class="btn btn-sm btn-secondary" data-bs-dismiss="modal" x-on:click="$store.global.clearForm('customerCateForm')">Close</button>
                <button type="button" class="btn btn-sm btn-primary" x-on:click="$store.customer.crudAction(event)">Save</button>
            </div>
        </div>
    </div>
</div>
    ...
    <script>
        ...
        document.addEventListener('alpine:init', () => {
            Alpine.store('customer', {
                ...
                store(event) {
                    let formData = new FormData(document.getElementById('customerCateForm'))
                    clearFlash()
